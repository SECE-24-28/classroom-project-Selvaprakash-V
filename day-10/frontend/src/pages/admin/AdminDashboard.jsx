...
import { FiUsers, FiZap, FiCreditCard, FiTrendingUp, FiArrowRight, FiActivity, FiPhone } from 'react-icons/fi';
import Layout from '../../components/Layout';

const AdminDashboard = () => {
  // Use useMemo to compute stats and transactions
  const { stats, recentTransactions } = useMemo(() => {
    const users = JSON.parse(localStorage.getItem('rechargex_users') || '[]');
    const transactions = JSON.parse(localStorage.getItem('rechargex_transactions') || '[]');
    const plans = JSON.parse(localStorage.getItem('rechargex_plans') || '[]');
  ...
  const statCards = [
    ...
    { icon: FiZap, label: 'Active Plans', value: stats.totalPlans, color: 'green', to: '/admin/plans' },
    { icon: FiCreditCard, label: 'Transactions', value: stats.totalTransactions, color: 'orange', to: '/admin/transactions' },
    { icon: FiTrendingUp, label: 'Total Revenue', value: `₹${stats.totalRevenue}`, color: 'amber', to: '/admin/transactions' },
  ];

  const quickActions = [
    { label: 'Manage Plans', to: '/admin/plans', icon: FiZap },
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400">
                          {transaction.operator} • {transaction.planName}
                        </p>
                      </div>
                    </div>
